...
from icl_tasks import LinguisticTask


def calculate_accuracy(task: LinguisticTask, predictions: List[str], expected_outputs: List[str]) -> List[bool]:
    correct = _evaluate_predictions(task, predictions, expected_outputs)
    accuracy = correct.mean()
    return accuracy


def calculate_accuracy_on_datasets(task: LinguisticTask, predictions: List[str], datasets: List[FewShotDataset]) -> List[bool]:
    expected_outputs = [dataset.test_output for dataset in datasets]
    return calculate_accuracy(task, predictions, expected_outputs)


def print_evaluation_summary(task: LinguisticTask, predictions: List[str], datasets: List[FewShotDataset]) -> None:
    expected_outputs = [dataset.test_output for dataset in datasets]
...
def _evaluate_predictions(task: LinguisticTask, predictions: List[str], expected_outputs: List[str]) -> List[bool]:
